...
def transmit_firmware(file_path):
    ...
    try:
        ...
        # 2. 파일 크기 전송 (빅 엔디안으로 변환)
        file_size = os.path.getsize(file_path)
        size_bytes = file_size.to_bytes(4, byteorder='big')  # 빅 엔디안으로 변환
        ...
        for chunk in chunks:
            ...
            # 빅 엔디안으로 변환한 데이터를 전송
            chunk_big_endian = bytearray()
            for i in range(0, len(chunk), 2):
                if i + 1 < len(chunk):
                    # 2바이트 단위로 빅 엔디안으로 변경
                    chunk_big_endian.append(chunk[i+1])
                    chunk_big_endian.append(chunk[i])
                else:
                    # 홀수 길이는 마지막 바이트를 그대로 추가
                    chunk_big_endian.append(chunk[i])

            transmitter.transmit_data(id=0x70, data=bytes(chunk_big_endian))
            sent_chunks += 1
...
# failed
# 빅 엔디안은 다 보내지긴 함
# 리틀(kvaser_trans.py)은 안됨
# 

# can으로 .bin을 보내고 
# stm32에서 해당 스택 위치로 다음 부트시퀀스 변환
